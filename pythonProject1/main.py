...
def validate_facts_against_rules(facts, rules):
    valid_facts = []
    for fact in facts:
        for rule in rules:
            if 'if' in rule and 'then' in rule and fact in rule['if']:
...
time_start = time()
valid_facts = validate_facts_against_rules(facts, rules)
print("%d facts validated vs %d rules in %f seconds" % (len(valid_facts), N, time()-time_start))